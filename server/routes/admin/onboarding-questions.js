...
module.exports = withAdmin(async (req, res) => {
  if (req.method === "GET") {
    const questions = await prisma.onboardingQuestion.findMany({ orderBy: { order: "asc" } });
    res.status(200).json(questions);
    return;
  }

  if (req.method === "POST") {
    const { question, type, options, order } = req.body || {};
    if (!question || !type) {
    ...
    const created = await prisma.onboardingQuestion.create({
      data: { question, type, options: options || null, order: nextOrder },
    });
